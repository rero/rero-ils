...
def test_deduplications_title_score():
    ...
    def create(title):
        return {"title": [{"type": "bf:Title", "mainTitle": [{"value": title}], "_text": title}]}
...
def test_series_statement_score():
    ...
    def create(statement):
        return {"seriesStatement": [{"_text": [{"language": "default", "value": statement}]}]}

    document = create("harry potter")
    assert Deduplication.get_series_statement_score(document, document) == 1
    assert Deduplication.get_series_statement_score(document, create("Harry   Potter ")) == 1
    # needs exact match
...
def test_edition_statement_score():
    ...
    def create(statement):
        return {"editionStatement": [{"_text": [{"language": "default", "value": statement}]}]}
...
def test_document_type_score():
    ...
    assert Deduplication.get_doc_type_score(create(["docmaintype_article"]), create(["docmaintype_article"])) == 1
    assert Deduplication.get_doc_type_score(create(["docmaintype_article"]), create(["docmaintype_book"])) == 0
    assert (
...
def test_publication_date_score():
    ...
    assert Deduplication.get_publication_date_score({"sort_date_old": 2024}, {"sort_date_old": 2024}) == 1
    assert Deduplication.get_publication_date_score({"sort_date_old": 2024}, {"sort_date_old": 2023}) == 0
...
def test_provision_activity_score():
    ...
    def create(prov):
        return {"provisionActivity": [{"_text": [{"language": "default", "value": prov}]}]}

    document = create("Paris : M. Lévy, 1860")
    assert Deduplication.get_provision_activity_score(document, document) == 1
    assert Deduplication.get_provision_activity_score(document, create("Paris :  m. Levy, 1860")) == 1
    assert Deduplication.get_provision_activity_score(document, create("Paris, 1860")) > 0
...
def test_extent_score():
    ...
    assert Deduplication.get_extent_score({"extent": "32 p., vol. 45"}, {"extent": "32 p."}) == 0.5
    assert Deduplication.get_extent_score({"extent": "32 p., vol. 45"}, {"extent": "volume 45, 32 p."}) == 1
...
def test_responsibility_score():
    ...
    assert Deduplication.get_responsibility_score(document, create("J. K. Rowling")) == 1
