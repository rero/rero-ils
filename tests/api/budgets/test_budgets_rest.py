...
def test_budgets_permissions(client, budget_2020_martigny, json_header):
    ...
    res, _ = postdata(client, "invenio_records_rest.budg_list", {})
    assert res.status_code == 401
...
def test_budgets_post_put_delete(client, budget_2018_martigny, json_header):
    """Test record retrieval."""
    # Create record / POST
    ...
    res, data = postdata(client, "invenio_records_rest.budg_list", budget_2018_martigny)
...
def test_budgets_can_delete(client, budget_2020_martigny, acq_account_fiction_martigny):
...
def test_budget_secure_api_create(
    client,
    json_header,
    ...
    res, _ = postdata(client, post_entrypoint, budget_2019_martigny)
    assert res.status_code == 403

    del budget_2020_martigny["pid"]
    res, _ = postdata(client, post_entrypoint, budget_2020_martigny)
    assert res.status_code == 403
...
def test_budget_secure_api_update(
    client,
...
def test_budget_secure_api_delete(
    client,
    budget_2017_martigny,
    librarian_martigny,
    librarian_sion,
