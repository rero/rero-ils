...
def test_local_fields_get(client, local_field_martigny):
    ...
    list_url = url_for("invenio_records_rest.lofi_list", pid=local_field_martigny.pid)
    res = client.get(list_url)
    assert res.status_code == 200
    data = get_json(res)
    assert data["hits"]["hits"][0]["metadata"] == local_field_martigny.replace_refs()
...
def test_local_fields_post_put_delete(
    ...
    res = client.put(item_url, data=json.dumps(new_lofi), headers=json_header)
    assert res.status_code == 200
    data = get_json(res)
