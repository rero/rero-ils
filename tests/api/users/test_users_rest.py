...
def test_users_post_put(
    ...
    res, data = postdata(client, "api_users.users_list", user_data_tmp)
    ...
    # test get
    res = client.get(url_for("api_users.users_item", id=2))
    assert res.status_code == 200
    user = get_json(res)
    assert user["id"] == 2
    assert user["metadata"]["first_name"] == user_data_tmp.get("first_name")
    ...
    res = client.put(
        url_for("api_users.users_item", id=2),
...
def test_users_search_api(
    ...
    res = client.get(url_for("api_users.users_list"))
    assert res.status_code == 200
    hits = get_json(res)
    ...
    res = client.get(url_for("api_users.users_item", id=p_martigny.user.id))
    assert res.status_code == 200
    record = get_json(res)
    assert patron_martigny["username"] == record.get("metadata", []).get("username")
