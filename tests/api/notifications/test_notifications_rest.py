...
from rero_ils.modules.notifications.models import NotificationStatus, NotificationType
...
def test_notification_secure_api(
    ...
    res, _ = postdata(client, post_entrypoint, notif)
...
def test_notifications_post_put_delete(
    ...
    notif = Notification.create(record, dbcommit=True, reindex=True, delete_pid=True)
    assert notif == record
    NotificationsSearch.flush_and_refresh()
    pid = notif.get("pid")
    ...
    # Create record / POST
    new_record["pid"] = "x"
    res, data = postdata(client, "invenio_records_rest.notif_list", new_record)
    assert res.status_code == 201
...
def test_recall_notification(
    ...
    assert not loan.is_notified(notification_type=NotificationType.RECALL, counter=1)
    assert not loan.is_notified(notification_type=NotificationType.AVAILABILITY)
    assert not get_notification(loan, NotificationType.AVAILABILITY)
    assert not get_notification(request_loan, NotificationType.REQUEST)
    assert not request_loan.is_notified(notification_type=NotificationType.REQUEST)
...
def test_cancel_notifications(
    ...
    notification = get_notification(loan, NotificationType.AVAILABILITY)
    assert notification and notification["status"] == NotificationStatus.CANCELLED
    assert len(mailbox) == 0
...
def test_reminder_notifications_after_extend(
    ...
    create_notifications(types=[NotificationType.DUE_SOON], tstamp=process_date)
    counter = (
        NotificationsSearch()
        .filter("term", context__loan__pid=loan.pid)
        .filter("term", notification_type=NotificationType.DUE_SOON)
