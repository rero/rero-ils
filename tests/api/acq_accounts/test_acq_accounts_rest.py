...
def test_acq_accounts_permissions(client, acq_account_fiction_martigny, json_header):
    """Test record retrieval."""
...
def test_acq_accounts_post_put_delete(
    ...
    res = client.put(item_url, data=json.dumps(acc_data), headers=json_header)
    data = get_json(res)
    assert res.status_code == 200
...
def test_acq_account_secure_api(
    client,
    json_header,
    acq_account_fiction_martigny,
    librarian_martigny,
...
def test_acq_account_secure_api_create(
    client,
    json_header,
    ...
    res, _ = postdata(client, post_entrypoint, acq_account_books_saxon_data)
    assert res.status_code == 403
...
def test_acq_account_secure_api_update(
    client,
...
def test_acq_account_secure_api_delete(
    client,
    acq_account_books_martigny,
    librarian_martigny,
    librarian_sion,
