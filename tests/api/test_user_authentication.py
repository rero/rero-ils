...
def test_login(client, patron_sion, default_user_password):
    ...
    res, _ = postdata(
        ...
        {"email": "not@exist.com", "password": ""},
    )
    assert res.status_code == 400
    data = get_json(res)
    assert data["message"] == "INVALID_USER_OR_PASSWORD"

    # wrong password
    res, _ = postdata(
        client,
        "invenio_accounts_rest_auth.login",
        {"email": patron_sion.get("email"), "password": "bad"},
    )
    assert res.status_code == 400
    data = get_json(res)
    ...
    res, _ = postdata(
        client,
        "invenio_accounts_rest_auth.login",
        {"email": patron_sion.get("email"), "password": default_user_password},
    )
    data = get_json(res)
    ...
    res, _ = postdata(
        client,
        "invenio_accounts_rest_auth.login",
        {"email": patron_sion.get("username"), "password": default_user_password},
    )
    data = get_json(res)
    assert res.status_code == 200
    assert data.get("id")
...
def test_patron_reset_notice(patron_martigny, mailbox):
    ...
    assert re.search(r"Your password has been successfully reset.", mailbox[0].body)
    assert re.search(r"Best regards", mailbox[0].body)
