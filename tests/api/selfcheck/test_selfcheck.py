...
from rero_ils.modules.selfcheck.api import (
    authorize_patron,
    enable_patron,
    item_information,
    patron_information,
...
def test_authorize_patron(selfcheck_patron_martigny, default_user_password):
    ...
    selfcheck_patron_martigny = Patron.get_record_by_pid(selfcheck_patron_martigny.pid)
    response = authorize_patron(
        selfcheck_patron_martigny.get("patron", {}).get("barcode")[0],
        default_user_password,
    )
...
def test_system_status(selfcheck_librarian_martigny):
    ...
    assert response.get("institution_id") == selfcheck_librarian_martigny.library_pid
...
def test_enable_patron(selfcheck_patron_martigny):
    ...
    assert response.get("language") == ptrn.patron["communication_language"]
...
def test_patron_information(
    ...
    pending_request_loan_pid = get_json(res)["action_applied"]["request"]["pid"]
    assert res.status_code == 200
...
def test_item_information(
    ...
    patron_barcode = selfcheck_patron_martigny.get("patron", {}).get("barcode")[0]
...
def test_selfcheck_circulation(
    client,
    selfcheck_librarian_martigny,
    document,
    librarian_martigny,
    librarian2_martigny,
    loc_public_martigny,
    ...
    assert "Error encountered: item not found" in checkout.get("screen_messages")
    ...
    assert "Error encountered: item not found" in checkin.get("screen_messages")
