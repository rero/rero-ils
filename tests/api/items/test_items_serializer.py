...
def test_serializers(
    ...
    item_url = url_for("invenio_records_rest.item_item", pid_value=item_lib_fully.pid)
    ...
    item_type.commit()
    item_type.reindex()
    item["temporary_item_type"] = {"$ref": get_ref_for_pid("itty", item_type.pid)}
    item.commit()
    item.reindex()
    list_url = url_for("invenio_records_rest.item_list", q=f"pid:{item.pid}")
    response = client.get(list_url, headers=rero_json_header)
    data = response.json["hits"]["hits"]
    assert circulation == data[0]["metadata"]["item_type"]["circulation_information"]
