...
def test_documents_import_bnf_ean(
    ...
    assert get_json(res) == {
        "aggregations": {},
        "hits": {"hits": [], "remote_total": 0, "total": 0},
    }
...
def test_documents_import_loc_isbn(
    ...
    res = client.get(url_for("api_imports.import_loc", q="", no_cache=1))
    assert res.status_code == 200
    assert get_json(res) == {
        "aggregations": {},
        "hits": {"hits": [], "remote_total": 0, "total": 0},
    ...
    mock_get.return_value = mock_response(content=loc_anywhere_all_samuelson)
    res = client.get(url_for("api_imports.import_loc", q="samuelson", no_cache=1))
    assert res.status_code == 200
    unfiltered_total = get_json(res)["hits"]["remote_total"]
    assert get_json(res)
...
def test_documents_import_dnb_isbn(
    ...
    mock_get.return_value = mock_response(content=dnb_isbn_3858818526)
    res = client.get(url_for("api_imports.import_dnb", q="3858818526", no_cache=1))
    assert res.status_code == 200
    res_j = get_json(res)
    data = res_j.get("hits").get("hits")[0].get("metadata")
    data.update(
    ...
    res = client.get(url_for("api_imports.import_dnb", q="", no_cache=1))
    assert res.status_code == 200
    assert get_json(res) == {
        "aggregations": {},
        "hits": {"hits": [], "remote_total": 0, "total": 0},
    ...
    mock_get.return_value = mock_response(content=dnb_samuelson)
    res = client.get(url_for("api_imports.import_dnb", q="samuelson, paul", no_cache=1))
    assert res.status_code == 200
    unfiltered_total = get_json(res)["hits"]["remote_total"]
    assert get_json(res)

    res = client.get(url_for("api_imports.import_dnb", q="samuelson, paul", year=2019))
    assert res.status_code == 200
    assert get_json(res)["hits"]["total"] < unfiltered_total
...
def test_documents_import_slsp_isbn(
    ...
    res = client.get(url_for("api_imports.import_slsp", q="", no_cache=1))
    assert res.status_code == 200
    assert get_json(res) == {
        "aggregations": {},
        "hits": {"hits": [], "remote_total": 0, "total": 0},
    ...
    mock_get.return_value = mock_response(content=slsp_samuelson)
    res = client.get(url_for("api_imports.import_slsp", q="samuelson", no_cache=1))
    assert res.status_code == 200
    unfiltered_total = get_json(res)["hits"]["remote_total"]
    assert get_json(res)
    ...
    mock_get.return_value = mock_response(content=slsp_recordid_9910137)
    res = client.get(
        url_for(
...
def test_documents_import_ugent_isbn(
    ...
    res = client.get(url_for("api_imports.import_ugent", q="", no_cache=1))
    assert res.status_code == 200
    assert get_json(res) == {
        "aggregations": {},
        "hits": {"hits": [], "remote_total": 0, "total": 0},
    ...
    mock_get.return_value = mock_response(content=ugent_samuelson)
    res = client.get(url_for("api_imports.import_ugent", q="samuelson", no_cache=1))
    assert res.status_code == 200
    unfiltered_total = get_json(res)["hits"]["remote_total"]
    assert get_json(res)
...
def test_documents_import_kul_isbn(
    ...
    res = client.get(url_for("api_imports.import_kul", q="", no_cache=1))
    assert res.status_code == 200
    assert get_json(res) == {
        "aggregations": {},
        "hits": {"hits": [], "remote_total": 0, "total": 0},
    ...
    mock_get.return_value = mock_response(content=kul_samuelson)
    res = client.get(url_for("api_imports.import_kul", q="samuelson", no_cache=1))
    assert res.status_code == 200
    unfiltered_total = get_json(res)["hits"]["remote_total"]
    assert get_json(res)
    ...
    mock_get.return_value = mock_response(content=kul_recordid_9992876296301471)
    res = client.get(
        url_for(
...
def test_documents_import_bnf_errors(mock_get, client, librarian_martigny):
    ...
    mock_get.return_value = mock_response(content=b"", status=429)
    res = client.get(url_for("api_imports.import_bnf", q="ean:any:123", no_cache=1))
