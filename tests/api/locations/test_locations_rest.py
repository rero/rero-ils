...
def test_location_pickup_locations(
    ...
    pickup_locations = Location.get_pickup_location_pids(patron_pid=patron_martigny.pid)
    assert set(pickup_locations) == {"loc1", "loc3", "loc5"}
    pickup_locations = Location.get_pickup_location_pids(patron_pid=patron_sion.pid)
...
def test_locations_get(client, loc_public_martigny, lib_martigny, org_martigny):
    ...
    assert (
        "$ref" not in resolved_data["library"]
        and lib_martigny.pid == resolved_data["library"]["pid"]
        and "lib" in resolved_data["library"]["type"]
    )
...
def test_locations_post_put_delete(
    ...
    # Create record / POST
    location_data["pid"] = "1"
    res, data = postdata(client, "invenio_records_rest.loc_list", location_data)
...
def test_filtered_locations_get(client, librarian_martigny, librarian_sion, locations):
...
def test_location_secure_api_create(
    client,
    lib_fully,
    lib_martigny,
    librarian_martigny,
    ...
    res, _ = postdata(client, post_entrypoint, fake_location_data)
    assert get_json(res) == {
        "status": 400,
...
def test_location_serializers(client, locations, librarian_martigny, rero_json_header):
