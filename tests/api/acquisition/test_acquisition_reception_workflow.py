...
def test_acquisition_reception_workflow(
    ...
    #      - check order (status)
    #      - check notification
    address = vendor_martigny.get_contact(VendorContactType.DEFAULT).get("email")
    emails = [
    ...
    res, data = postdata(
        ...
        url_data={"receipt_pid": receipt_1.pid},
    )
    assert res.status_code == 200
    ...
    for order_line in [
        ...
        {"line": order_line_2, "status": AcqOrderLineStatus.ORDERED, "received": 0},
        {"line": order_line_3, "status": AcqOrderLineStatus.CANCELLED, "received": 0},
        {"line": order_line_4, "status": AcqOrderLineStatus.ORDERED, "received": 0},
        {"line": order_line_5, "status": AcqOrderLineStatus.ORDERED, "received": 0},
        {"line": order_line_6, "status": AcqOrderLineStatus.CANCELLED, "received": 0},
    ]:
        line = AcqOrderLine.get_record_by_pid(order_line.get("line").pid)
        assert line.status == order_line.get("status")
    ...
    res, data = postdata(client, "api_receipt.lines", data=data, url_data={"receipt_pid": receipt_2.pid})
