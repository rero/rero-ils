...
def test_patron_pending_subscription(
    client,
    patron_type_grown_sion,
    patron_sion,
    librarian_sion,
    ...
    payment["operator"] = {"$ref": get_ref_for_pid("patrons", librarian_sion.pid)}
...
def test_patrons_circulation_informations(
    ...
    assert data["statistics"] == {"ill_requests": 1}
    ...
    url = url_for("api_patrons.patron_circulation_informations", patron_pid="dummy_pid")
    res = client.get(url)
