...
def test_create_holdings_with_pattern(
    ...
    holding_lib_martigny_data["patterns"] = pattern_yearly_one_level_data["patterns"]
...
def test_holding_pattern_preview_api(client, pattern_yearly_one_level_data, librarian_martigny):
    ...
    # test preview by default 10 issues returned
    res, data = postdata(client, "api_holding.pattern_preview", {"data": patterns, "size": 15})
    assert res.status_code == 200
...
def test_irregular_issue_creation_update_delete_api(client, holding_lib_martigny_w_patterns, librarian_martigny):
    ...
    item = {
        ...
        "type": "issue",
        "organisation": {"$ref": get_ref_for_pid("org", holding.organisation_pid)},
    }
    res, data = postdata(client, "invenio_records_rest.item_list", item)
    assert res.status_code == 201
