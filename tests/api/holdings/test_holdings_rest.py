...
def test_holding_can_delete_and_utils(
    ...
    assert (
        next(iter(Holding.get_holdings_pid_by_document_pid(document.pid)))
...
def test_holdings_items_filter(
    ...
    assert len(holding_lib_martigny.get_items_filter_by_viewcode("org2")) == 0
...
def test_holdings_post_put_delete(
    client,
