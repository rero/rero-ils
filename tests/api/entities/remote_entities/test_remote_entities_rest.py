...
def test_remote_entities_permissions(client, entity_person, json_header):
    ...
    item_url = url_for("invenio_records_rest.rement_item", pid_value="ent_pers")
...
def test_remote_entities_get(client, entity_person):
    ...
    assert data["hits"]["hits"][0]["metadata"] == entity_person.dumps(indexer_dumper)
...
def test_remote_search_proxy(
    ...
    assert response.json["message"] == f"Unable to find a MEF factory for {category}"
