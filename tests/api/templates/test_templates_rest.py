...
def test_template_secure_api_create(
    ...
    ptrn_tmpl["data"].update({"pid": "toto", "user_id": "toto"})
    ptrn_tmpl["data"]["patron"].update({"subscriptions": "toto", "barcode": ["toto"]})
...
def test_template_secure_api_update(
    ...
    # Reset data
    templ_doc_private_martigny.update(original_data, dbcommit=True, reindex=True)
...
def test_template_update_visibility(
    ...
    with mock.patch("flask_login.utils._get_user", magic_mock):
        ...
        tmpl["creator"]["$ref"] = get_ref_for_pid("ptrn", librarian2_martigny.pid)
    ...
    res = client.put(record_url, data=json.dumps(tmpl), headers=json_header)
    assert res.status_code == 200
