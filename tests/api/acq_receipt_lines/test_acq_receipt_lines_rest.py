...
def test_acq_receipt_lines_permissions(client,
    ...
    res = client.put(
        url_for('invenio_records_rest.acrl_item', pid_value='acrl1'),
        data={},
        headers=json_header
    )
...
def test_acq_receipt_line_secure_api(client, json_header,
                                     acq_receipt_line_1_fiction_martigny,
                                     librarian_martigny, librarian_sion):
    """Test acq receipt line secure api access."""
    # Martigny
    login_user_via_session(client, librarian_martigny.user)
    record_url = url_for('invenio_records_rest.acrl_item',
                         pid_value=acq_receipt_line_1_fiction_martigny.pid)
...
def test_acq_receipt_line_secure_api_create(
        client, json_header, org_martigny, vendor_martigny, vendor2_martigny,
        acq_receipt_line_1_fiction_martigny, librarian_martigny,
        librarian_sion, acq_receipt_line_fiction_saxon,
        system_librarian_martigny):
    """Test acq receipt line secure api create."""
    # Martigny
    login_user_via_session(client, librarian_martigny.user)
    post_entrypoint = 'invenio_records_rest.acrl_list'

    data = acq_receipt_line_fiction_saxon
    del acq_receipt_line_fiction_saxon['pid']
    res, _ = postdata(
        client,
        post_entrypoint,
        data
    ...
    data = deepcopy(acq_receipt_line_1_fiction_martigny)
    del data['pid']
    res, _ = postdata(
        client,
        post_entrypoint,
...
def test_acq_receipt_line_secure_api_update(client, org_sion, vendor_sion,
                                            acq_receipt_line_fiction_sion,
                                            librarian_martigny,
                                            librarian_sion, json_header):
    """Test acq receipt line secure api update."""
    # Sion
    login_user_via_session(client, librarian_sion.user)
    record_url = url_for('invenio_records_rest.acrl_item',
                         pid_value=acq_receipt_line_fiction_sion.pid)
    data = acq_receipt_line_fiction_sion
    notes = [{'content': 'test', 'type': AcqReceiptLineNoteType.STAFF}]
    data['notes'] = notes
    res = client.put(
        record_url,
