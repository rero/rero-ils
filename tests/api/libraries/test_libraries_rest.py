...
def test_libraries_post_put_delete(client, lib_martigny_data, json_header):
    ...
    res, data = postdata(client, "invenio_records_rest.lib_list", lib_martigny_data)
...
def test_library_never_open(lib_sion):
    ...
    open_exception = {
        ...
        "times": [{"end_time": "16:00", "start_time": "12:00"}],
    }
...
def test_library_secure_api(
    ...
    # Martigny
    login_user_via_session(client, librarian_martigny.user)
    record_url = url_for("invenio_records_rest.lib_item", pid_value=lib_martigny.pid)

    res = client.get(record_url)
    # a librarian is authorized to access its library record of its org
    assert res.status_code == 200

    record_url = url_for("invenio_records_rest.lib_item", pid_value=lib_fully.pid)
    res = client.get(record_url)
...
def test_library_secure_api_create(
    client,
    lib_martigny,
    lib_fully_data,
    librarian2_martigny,
...
def test_library_secure_api_update(
    client,
    lib_fully,
    lib_martigny,
    librarian_martigny,
    ...
    res = client.put(record_url, data=json.dumps(lib_martigny), headers=json_header)
    ...
    res = client.put(record_url, data=json.dumps(lib_fully), headers=json_header)
...
def test_library_secure_api_delete(
    client,
    lib_fully,
    lib_martigny,
    librarian2_martigny,
