...
def test_get_query_clause_with_relation(app):
    ...
    query = parse(
        "ind1 = 1 AND ind2 > 2 AND ind3 >= 3 AND ind4 < 4 AND ind5 <= 5 AND ind6 <> 6"
    )
    es_string = query.to_es()
    assert es_string == (
...
def test_prefixable_object():
    ...
    with pytest.raises(Diagnostic) as err:
        prefix_object.add_prefix("name", "identifier")
    assert str(err.value) == ("info:srw/diagnostic/1/45 [Malformed Query]: name")
...
def test_prefixed_object():
    ...
    assert str(err.value) == ('info:srw/diagnostic/1/14 [Malformed Query]: "tick"')
...
def test_term():
    ...
    assert str(err.value) == ("info:srw/diagnostic/1/27 [Malformed Query]: ")
    with pytest.raises(Diagnostic) as err:
        Term(">=", query="query")
