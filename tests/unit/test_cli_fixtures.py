...
def test_count(app, script_info):
    ...
    runner = CliRunner()
    result = runner.invoke(count_cli, [json_file_name], obj=script_info)
    assert result.exit_code == 0
    assert result.output.strip().split("\n")[1] == "Count: 2"

    runner = CliRunner()
    result = runner.invoke(count_cli, [json_file_name, "-l"], obj=script_info)
    assert result.exit_code == 0
    assert result.output.strip().split("\n")[1] == "Count: 2"
...
def test_create(
    mock_contributions_mef_get, app, script_info, entity_person_response_data
):
    """Test create cli."""
    json_file_name = join(dirname(__file__), "../data/documents.json")
