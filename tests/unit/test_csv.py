...
def test_create_csv(app, tmpdir, script_info):
    ...
    result = runner.invoke(
        create_csv, ["doc", json_file_name, tmp_dir_name, "-l", "-v"], obj=script_info
    )
    ...
    result = runner.invoke(
        bulk_load, ["doc", file_name_metadata, "-r"], obj=script_info
    )
    assert result.exit_code == 0
    ...
    result = runner.invoke(
        bulk_save, [tmp_dir_name, "-t", "xxx", "-t", "doc"], obj=script_info
    )
    assert result.exit_code == 0
    assert result.output.split("\n") == [
