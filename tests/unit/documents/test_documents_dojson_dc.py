...
def test_summary_note_dissertation_supplementarycontent_to_dc():
    ...
    record = {
        "summary": [
            {
                "label": [
                    {"value": "Fictions jeunesse: roman, jeux vid\u00e9o, imagination"}
                ]
            }
        ],
        "note": [
...
def test_language_to_dc():
    ...
    record = {"language": [{"type": "bf:Language", "value": "ain"}]}
    result = dublincore.do(record)
...
def test_provision_activity_to_dc():
        ...
        "provisionActivity": [
            {
                ...
                "statement": [
                    ...
                    {"type": "bf:Agent", "label": [{"value": "Listening Library"}]},
                    {"type": "Date", "label": [{"value": "2006"}]},
                ],
                "startDate": 2006,
                "place": [{"type": "bf:Place", "country": "xxu"}],
            }
        ]
    }
    result = dublincore.do(record)
    ...
    record = {
        "provisionActivity": [
            {
                "type": "bf:Publication",
                "statement": [{"label": [{"value": "1978"}], "type": "Date"}],
                "startDate": 1978,
...
def test_identified_by_to_dc():
    ...
    record = {
        "identifiedBy": [
            ...
            {"value": "Arbiter 142", "type": "bf:AudioIssueNumber"},
            {"value": "R005918723", "type": "bf:Local", "source": "RERO"},
        ]
    }
    result = dublincore.do(record)
    assert result == {
...
def test_subjects_to_dc(app):
    ...
    record = {
        "subjects": [
            {"type": "bf:Person", "preferred_name": "Preferred name"},
            {"type": "bf:Work", "title": "Title", "creator": "Creator"},
            {"type": "bf:Topic", "term": "Term"},
        ]
    }
    result = dublincore.do(record)
    assert result == {"subjects": ["Preferred name", "Creator. - Title", "Term"]}
