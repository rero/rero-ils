...
from rero_ils.modules.cli.index import (
    delete_queue,
    init_queue,
    purge_queue,
    reindex,
...
def test_cli_reindex_missing(app, org_sion_data):
    ...
    runner = CliRunner()
    res = runner.invoke(init_queue, ["-n", queue_name])
    ...
    runner = CliRunner()
    res = runner.invoke(run, ["-q", queue_name])
    assert res.output.strip().split("\n") == [
    ...
    queue_name = "test_queue"
    runner = CliRunner()
    res = runner.invoke(purge_queue, ["-n", queue_name])
    assert res.output.strip().split("\n") == [f"Queue has been purged: {queue_name} 0"]
    # - delete the new indexer queue
    queue_name = "test_queue"
    runner = CliRunner()
    res = runner.invoke(delete_queue, ["-n", queue_name])
