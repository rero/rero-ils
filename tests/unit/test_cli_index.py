...
def test_cli_reindex_missing(app, script_info, org_sion_data):
    ...
    runner = CliRunner()
    res = runner.invoke(
        reindex_missing, ["-t", "xxx", "-t", "org", "-v"], obj=script_info
    )
    ...
    runner = CliRunner()
    res = runner.invoke(reindex, ["-t", "org", "--yes-i-know"], obj=script_info)
    assert res.output.strip().split("\n") == [
    ...
    runner = CliRunner()
    res = runner.invoke(reindex, ["-t", "org", "--yes-i-know", "-d"], obj=script_info)
    assert res.output.strip().split("\n") == [
        "Indexing org: 1",
        f"1\t{org.id}\t{org.pid}",
    ]
    ...
    runner = CliRunner()
    res = runner.invoke(
        reindex, ["-t", "org", "-q", queue_name, "--yes-i-know"], obj=script_info
    )
    assert res.output.strip().split("\n") == [
