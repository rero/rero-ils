...
def mef_record_with_idref_rero(mef_record_with_idref_rero_data):
    """Mef record with idref rero."""
    if entity := RemoteEntity.get_record_by_pid(mef_record_with_idref_rero_data["pid"]):
        return entity
...
def mef_record_with_idref_gnd_data():
    ...
    return {
        ...
        "idref": {
            ...
            "relation_pid": {"type": "redirect_from", "value": "126899959"},
            "variant_access_point": [
                "UNIGE",
                "Academia Genevensis",
                "Académie de Genève",
...
def mef_record_with_idref_gnd(mef_record_with_idref_gnd_data):
    """Mef record with idref rero."""
    if entity := RemoteEntity.get_record_by_pid(mef_record_with_idref_gnd_data["pid"]):
        return entity
    entity = RemoteEntity.create(
