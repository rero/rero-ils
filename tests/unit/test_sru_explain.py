...
def test_explain(app):
    ...
    assert explain.number_of_records == app.config.get(
        'RERO_SRU_NUMBER_OF_RECORDS')
    assert explain.maximum_records == app.config.get(
        'RERO_SRU_MAXIMUM_RECORDS')
    assert explain.doc_type == 'doc'
    assert explain.index == app.config.get(
        'RECORDS_REST_ENDPOINTS', {}).get('doc', {}).get('search_index')
