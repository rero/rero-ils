...
def test_api_source(mock_get, app, capsys):
    ...
    harvest_records(
        name="test", url="http://test.com", signals=False, size=1000, max_results=1000
    )
    ...
    assert msg == (
        "Updated: url:http://test.com, mimetype:mimetype," " size:1000, comment:comment"
    )
...
def test_get_records(mock_get, app, capsys):
    ...
    for next_url, data in get_records(
        url="http://test.com", name="test", signals=False
    ):
        assert next_url
        assert data == [
    ...
    for next_url, data in get_records(
        url="http://test.com", name="test", from_date="1970-01-01", signals=False
    ):
        assert next_url
        assert data == [
