...
def test_auto_extend_task(item_on_loan_martigny_patron_and_loan_on_loan,
    ...
    # FIRST TEST: If automatic_renewal is not set in the policy, the loan is
    # not renewed
    ...
    # (because the unextended loand was filtered directly in the first query)
    assert result == (0, 0)
    assert item.status == ItemStatus.ON_LOAN
    assert not loan.get('auto_extend')
    assert not mailbox

    # SECOND TEST: If loan is not renewable (request exists), it is ignored
    ...
    # THIRD TEST:: Auto extend one extendable loan
