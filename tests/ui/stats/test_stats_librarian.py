...
def test_stats_librarian_active_patrons_by_postal_code(
    ...
    assert stat_for_librarian.active_patrons_by_postal_code(lib_martigny.pid) == {
        "1920": 1
    }

    # with new patrons
    assert (
        stat_for_librarian.active_patrons_by_postal_code("foo", new_patrons=True) == {}
    )
    assert stat_for_librarian.active_patrons_by_postal_code(
...
def test_stats_librarian_items_by_document_type_and_subtype(
    ...
    assert (
        stat_for_librarian.items_by_document_type_and_subtype(
            item_lib_martigny.library_pid
        )["docmaintype_book"]
        >= 1
    )
    assert (
