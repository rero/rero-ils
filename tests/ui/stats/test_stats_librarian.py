...
def test_stats_librarian_checkouts_for_transaction_library(
    ...
    assert stat_for_librarian.checkouts_for_transaction_library("foo") == 0
    assert stat_for_librarian.checkouts_for_transaction_library(lib_sion.pid) == 0
    assert stat_for_librarian.checkouts_for_transaction_library(lib_martigny.pid) == 1
...
def test_stats_librarian_checkouts_for_owning_library(
    ...
    """Test the number of circulation operation during the specified timeframe."""
    assert stat_for_librarian.checkouts_for_owning_library("foo") == 0
    assert stat_for_librarian.checkouts_for_owning_library(lib_sion.pid) == 0
    assert stat_for_librarian.checkouts_for_owning_library(lib_martigny.pid) == 1


def test_stats_librarian_active_patrons_by_postal_code(stat_for_librarian, loan_due_soon_martigny, lib_martigny):
    """Test the number of circulation operation during the specified timeframe."""
    assert stat_for_librarian.active_patrons_by_postal_code("foo") == {}
    assert stat_for_librarian.active_patrons_by_postal_code(lib_martigny.pid) == {"1920": 1}

    # with new patrons
    assert stat_for_librarian.active_patrons_by_postal_code("foo", new_patrons=True) == {}
    assert stat_for_librarian.active_patrons_by_postal_code(lib_martigny.pid, new_patrons=True) == {"1920": 1}
    stat = StatsForLibrarian()
    assert stat.active_patrons_by_postal_code(lib_martigny.pid, new_patrons=True) == {}


def test_stats_librarian_new_documents(stat_for_librarian, document_data, lib_martigny, librarian_martigny):
    ...
    with mock.patch(
        ...
        Document.create(data=document_data, delete_pid=True, dbcommit=False, reindex=False)
...
def test_stats_librarian_validated_requests(stat_for_librarian, lib_sion, loan_validated_sion):
...
def test_stats_librarian_new_items_by_location(stat_for_librarian, item_lib_martigny, loc_public_martigny):
    ...
    assert (
        stat_for_librarian.new_items_by_location(item_lib_martigny.library_pid)[f"{loc['code']} - {loc['name']}"] >= 1
    )
    stat = StatsForLibrarian()
    assert stat.new_items_by_location(item_lib_martigny.library_pid) == {}


def test_stats_librarian_items_by_document_type_and_subtype(stat_for_librarian, item_lib_martigny, loc_public_martigny):
    ...
    assert stat_for_librarian.items_by_document_type_and_subtype(item_lib_martigny.library_pid)["docmaintype_book"] >= 1
    assert (
        stat_for_librarian.items_by_document_type_and_subtype(item_lib_martigny.library_pid)["docsubtype_other_book"]
        >= 1
    )


def test_stats_librarian_loans_of_transaction_library_by_item_location(
    stat_for_librarian, loan_due_soon_martigny, lib_martigny, loc_public_martigny
):
    """Test the number of circulation operation during the specified timeframe."""
    ...
    res = stat_for_librarian.loans_of_transaction_library_by_item_location(lib_martigny.pid)
