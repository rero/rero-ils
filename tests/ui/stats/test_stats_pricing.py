...
def test_stats_pricing_number_of_documents(stat_for_pricing, item_lib_martigny):
    ...
    assert stat_for_pricing.number_of_documents(item_lib_martigny.library_pid) == 1
...
def test_stats_pricing_number_of_libraries(stat_for_pricing, lib_martigny):
    ...
    assert stat_for_pricing.number_of_libraries(lib_martigny.organisation_pid) == 1


def test_stats_pricing_number_of_librarians(stat_for_pricing, librarian_martigny):
...
def test_stats_pricing_number_of_items(stat_for_pricing, item_lib_martigny):
    ...
    assert stat_for_pricing.number_of_items(item_lib_martigny.library_pid) >= 1


def test_stats_pricing_number_of_new_items(stat_for_pricing, item_lib_martigny):
    """Test the number of new created items during the specified timeframe."""
    assert stat_for_pricing.number_of_new_items("foo") == 0
    # loans used in previous tests can adds some items
    assert stat_for_pricing.number_of_new_items(item_lib_martigny.library_pid) >= 1
    from rero_ils.modules.stats.api.pricing import StatsForPricing
...
def test_stats_pricing_number_of_patrons(stat_for_pricing, patron_martigny):
    """Test the number of users with a librarian role."""
...
def test_stats_pricing_number_of_new_patrons(stat_for_pricing, patron_martigny):
    ...
    # today item creation is excluded
    stat = StatsForPricing()
    assert stat.number_of_new_patrons(patron_martigny.organisation_pid) == 0


def test_stats_pricing_files(stat_for_pricing, lib_martigny, document_with_files):
