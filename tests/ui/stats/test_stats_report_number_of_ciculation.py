...
def test_stats_report_circulation_trigger(
    ...
    # fixtures
    for trigger in ["checkin", "checkout", "extend", "request", "validate_request"]:
        es.index(
...
def test_stats_report_number_of_checkins(
    ...
    es.index(
        ...
        body={
            ...
            "loan": {
                "trigger": "checkin",
                "item": {"document": {"type": "ebook"}, "library_pid": lib_sion.pid},
    ...
    cfg = {
        "library": {"$ref": "https://bib.rero.ch/api/libraries/lib1"},
        "is_active": True,
        "category": {"indicator": {"type": "number_of_checkins"}},
    }
    assert StatsReport(cfg).collect() == [[2]]
    ...
    cfg = {
        "library": {"$ref": "https://bib.rero.ch/api/libraries/lib1"},
        "is_active": True,
        "category": {"indicator": {"type": "number_of_checkins", "period": "year"}},
    }
    ...
    assert StatsReport(cfg).collect() == [
        ["", label_loc_pub_martigny_bourg, label_loc_pub_martigny],
        ["2023-01", 0, 1],
        ["2024-01", 1, 0],
    ]
    ...
    assert StatsReport(cfg).collect() == [["docsubtype_other_book", 1], ["ebook", 1]]
    ...
    assert StatsReport(cfg).collect() == [["sip2", 1], ["system", 1]]
