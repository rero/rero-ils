...
from rero_ils.modules.items.dumpers import (
    CirculationActionDumper,
...
def test_item_action_circulation_dumper(
    ...
    item["temporary_location"] = {"pid": "loc15", "type": "loc"}

    data = item.dumps(CirculationActionDumper())

    # temporary location name
    assert data["temporary_location"]["name"] == loc_public_martigny_bourg.get("name")
