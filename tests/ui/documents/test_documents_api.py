...
def test_document_create_with_mef(
    ...
    with pytest.raises(ValidationError):
        doc = Document.create(data={}, delete_pid=False, dbcommit=True, reindex=True)
    ...
    doc = Document.create(data=data, delete_pid=False, dbcommit=False, reindex=False)
    doc.reindex()
    flush_index(DocumentsSearch.Meta.index)
    with pytest.raises(ValidationError):
...
def test_document_linked_subject(
    ...
    assert "Inhibiteurs enzymatiques" in subject["entity"]["variant_access_point"]
...
def test_document_contribution_resolve_exception(
    es_clear, db, mef_agents_url, document_data_ref
):
