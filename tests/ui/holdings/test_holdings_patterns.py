...
def test_patterns_functions(holding_lib_martigny_w_patterns, holding_lib_martigny):
...
def test_receive_regular_issue(holding_lib_martigny_w_patterns, tomorrow):
    ...
    assert issue_status_date.strftime("%Y-%m-%d") == datetime.now().strftime("%Y-%m-%d")
    ...
    new_issue_status_date = ciso8601.parse_datetime(new_issue.issue_status_date)
    ...
    with mock.patch(
        ...
        assert holding.reasons_not_to_delete() == {
            "others": [{"message": "has {{value}} items with {{name}} attached", "data": {"name": "fees", "value": 3}}]
        }
    with mock.patch(
        "rero_ils.modules.items.api.Item.get_links_to_me",
        ...
        assert holding.reasons_not_to_delete() == {
            "others": [
                {"message": "has {{value}} items with {{name}} attached", "data": {"name": "collections", "value": 3}}
            ]
        }
...
def test_patterns_yearly_one_level_with_label(
    holding_lib_martigny_w_patterns, pattern_yearly_one_level_with_label_data
):
...
def test_patterns_quarterly_two_levels_with_season(
    ...
    holding = holding_lib_martigny_w_patterns
    holding = Holding.get_record_by_pid(holding.pid)
    holding["patterns"] = pattern_quarterly_two_levels_with_season_data["patterns"]
    # test first issue
    assert holding.next_issue_display_text == "année 2019 no 277 printemps 2018"
    holding.increment_next_prediction()
    assert holding.next_issue_display_text == "année 2019 no 278 été 2018"
    for _ in range(25):
        holding.increment_next_prediction()
    assert holding.next_issue_display_text == "année 2025 no 303 automne 2024"
    # test preview
    issues = holding.prediction_issues_preview(13)
...
def test_patterns_half_yearly_one_level(holding_lib_martigny_w_patterns, pattern_half_yearly_one_level_data):
    ...
    holding["patterns"] = pattern_half_yearly_one_level_data["patterns"]
...
def test_patterns_bimonthly_every_two_months_one_level(
    holding_lib_martigny_w_patterns, pattern_bimonthly_every_two_months_one_level_data
):
    """Test pattern quarterly_two_levels."""
    holding = holding_lib_martigny_w_patterns
    holding = Holding.get_record_by_pid(holding.pid)
    holding["patterns"] = pattern_bimonthly_every_two_months_one_level_data["patterns"]
    # test first issue
...
def test_patterns_half_yearly_two_levels(holding_lib_martigny_w_patterns, pattern_half_yearly_two_levels_data):
    ...
    holding = holding_lib_martigny_w_patterns
    holding = Holding.get_record_by_pid(holding.pid)
    holding["patterns"] = pattern_half_yearly_two_levels_data["patterns"]
    # test first issue
...
def test_bimonthly_every_two_months_two_levels(
    holding_lib_martigny_w_patterns, pattern_bimonthly_every_two_months_two_levels_data
):
    """Test pattern bimonthly_every_two_months_two_levels."""
    holding = holding_lib_martigny_w_patterns
    holding = Holding.get_record_by_pid(holding.pid)
    holding["patterns"] = pattern_bimonthly_every_two_months_two_levels_data["patterns"]
    # test first issue
...
def test_intervals_and_expected_dates(holding_lib_martigny_w_patterns):
        ...
        "values": [
            {
                ...
                "levels": [{"number_name": "level_1", "starting_value": 1}],
            }
        ],
    }
    ...
    for issue in issues:
        expected_date = datetime.strptime(issue.get("expected_date"), "%Y-%m-%d")
...
def test_regular_issue_creation_update_delete_api(
    client, holding_lib_martigny_w_patterns, loc_public_martigny, lib_martigny
):
