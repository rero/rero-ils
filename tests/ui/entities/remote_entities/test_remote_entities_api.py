...
from rero_ils.modules.utils import get_schema_for_resource, get_timestamp, set_timestamp
...
def test_sync_entities_aborted_run_keeps_last_date(app, entity_person_data):
    """An aborted run is recorded, but does not advance the synchronization date."""
    entity = RemoteEntity.create(deepcopy(entity_person_data), dbcommit=True, reindex=True, delete_pid=True)
    RemoteEntitiesSearch.flush_and_refresh()
    last_date = datetime(2024, 1, 1)
    set_timestamp("sync_entities", start_timestamp=last_date)

    sync_entity = SyncEntity()
    error = mock_response(status=500, json_data=[{"pid": entity.pid}])
    with mock.patch("requests.Session.post", return_value=error), pytest.raises(requests.ConnectionError):
        sync_entity.sync(f"pid:{entity.pid}", from_date=last_date)

    data = get_timestamp("sync_entities")
    # the run is accounted for ...
    assert data["n_doc_updated"] == 0
    assert data["n_mef_updated"] == 0
    # ... but the window is kept, so the next run processes it again
    assert data["start_timestamp"] == last_date
