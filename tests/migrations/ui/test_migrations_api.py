...
from rero_ils.modules.migrations.api import Migration, MigrationError
...
def test_migrations_create(es_indices, lib_martigny):
    ...
    with pytest.raises(MigrationError.ValidationError):
        Migration(name="name", library_pid="2", status="bad").save()
    migration = Migration(name="bla", library_pid=lib_martigny.pid)
    ...
    assert Migration.get(migration.meta.id).name == "toto"
    migration.delete()
