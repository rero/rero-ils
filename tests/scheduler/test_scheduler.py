...
from rero_ils.schedulers import RedisScheduler, current_scheduler, \
    enable_tasks, info, init
...
def test_scheduler(app):
    ...
    enabled_task = display_tasks[0].replace(
        'enabled:False',
        'enabled:True'
    )
    ...
    test_task = display_tasks[0].replace(
        'kwargs:{}',
        "kwargs:{'test': 'test'}"
    )
...
def test_scheduler_cli(app, script_info):
    """Test scheduler cli."""
    display_tasks = [
        ('- bulk-indexer = rero_ils.modules.tasks.process_bulk_queue '
         '<freq: 1.00 hour> '
    ...
    runner = CliRunner()
    res = runner.invoke(init, ['-r', '-v'], obj=script_info)
    assert res.output.strip().split('\n') == [
        'Reset REDIS scheduler!',
        display_tasks[0]
    ]

    res = runner.invoke(init, ['-v'], obj=script_info)
    assert res.output.strip().split('\n') == [
        'Initalize REDIS scheduler!',
        display_tasks[0]
    ]

    res = runner.invoke(enable_tasks, ['-a', '-v'], obj=script_info)
    enabled_task = display_tasks[0].replace(
        'enabled:False',
        'enabled:True'
    )
    ...
    res = runner.invoke(
        enable_tasks, ['-v', '-n bulk-indexer', '-n dummy', '-d'],
        obj=script_info
    )
    assert res.output.strip().split('\n') == [
        'Scheduler tasks enabled:',
    ...
    res = runner.invoke(info, [], obj=script_info)
    assert res.output.strip().split('\n') == [
        'Scheduled tasks:',
        display_tasks[0]
    ]
