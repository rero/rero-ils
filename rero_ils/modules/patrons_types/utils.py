...
def save_patron_type(data, record_type, record_class, parent_pid):
    ...
    pid = data.get('pid')
    if pid:
        patron_type = record_class.get_record_by_pid(pid)
        patron_type.update(data, dbcommit=True, reindex=True)
    else:
        data['organisation_pid'] = current_patron.organisation.pid
        patron_type = record_class.create(data, dbcommit=True, reindex=True)
    RecordIndexer().client.indices.flush()

    _next = url_for('invenio_records_ui.ptty', pid_value=patron_type.pid)
    return _next, patron_type.pid
