...
class Term:
    ...
    def __init__(self, value, query, error_on_empty_term=ERROR_ON_EMPTY_TERM):
        ...
        if value != "":
            ...
            while idx > -1:
                if len(value) < idx + 2 or not value[idx + 1] in ["?", "\\", "*", "^"]:
                    diag = Diagnostic()
                    diag.code = 26
                    diag.details = value
                    diag.query = query
...
class CQLParser:
    ...
    def clause(self):
        ...
        if not sort and not is_boolean and not (self.next_token in [")", "(", ""]):
            index = IndexerType(self.current_token, self.parser.query)
            ...
            irt = RelatioSearchClauseType(index, relation, term, self.parser.query)
