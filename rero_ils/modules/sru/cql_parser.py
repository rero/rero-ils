...
class SearchClause(PrefixableObject):
    ...
    def __init__(self, ind, rel, term):
        """Constructor."""
...
class Term:
    ...
    def __init__(self, value, error_on_empty_term=ERROR_ON_EMPTY_TERM):
        """Constructor."""
        if value != '':
            # Unquoted literal
            if value in ['>=', '<=', '>', '<', '<>', '/', '=']:
...
class Boolean(ModifiableObject):
    ...
    def __init__(self, bool_value, mods=[]):
        """Constructor."""
...
class ModifierClause:
    ...
    def __init__(self, modifier_type, comp='', val=''):
        """Constructor."""
...
class CQLParser:
    ...
    def sort_query(self):
        ...
        while self.current_token:
            ...
            if self.current_token == ')':
                break
            index = IndexerType(self.current_token)
            self.fetch_token()
    ...
    def clause(self):
        ...
        if not sort and \
            ...
            if self.current_token == '':
                ...
                raise diag
            term = TermType(self.current_token)
            ...
            irt = RelatioSearchClauseType(
                IndexerType(SERVER_CHOISE_INDEX),
                RelationType(SERVER_CHOISE_RELATION),
                TermType(self.current_token)
            )
            self.fetch_token()
    ...
    def boolean(self):
        ...
        if self.current_token in BOOLEANS:
            bool_type = BooleanType(self.current_token)
            self.fetch_token()
    ...
    def relation(self):
        """Find relation."""
        self.current_token = self.current_token.lower()
        relation = RelationType(self.current_token)
        self.fetch_token()
...
def parse(query):
    ...
    lexer = CQLshlex(query_io_string)
