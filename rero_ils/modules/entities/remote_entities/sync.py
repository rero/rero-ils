...
class SyncEntity:
    ...
    def _entity_are_different(self, entity1, entity2):
        ...
        def remove_fields(entity):
            ...
            fields_to_remove = ["$schema", "organisation", "_created", "_updated"]
                ...
                try:
                    while chunk := list(islice(pids, chunk_size)):
                        ...
                        for mef_type, mef_pids in pids_by_type.items():
                            ...
                            # ask the mef server to return only the updated pids from a given date
    ...
    def end_sync(self, n_doc_updated, n_mef_updated, mef_errors, completed=True):
        """Add logging and cache information about the ending process.

        :param completed: if False the run stopped part-way: the counters and
            the errors are still recorded, but `start_timestamp` keeps its
            previous value so that `--from-last-date` retries the same window
            instead of skipping over the records left unprocessed.
        """
        self.logger.info("DONE: doc updated: %s, mef updated: %s.", n_doc_updated, n_mef_updated)
        if self.dry_run:
            return
        data = get_timestamp("sync_entities") or {}
        ...
        set_timestamp(
            ...
            errors=errors,
            start_timestamp=self.start_timestamp if completed else data.get("start_timestamp"),
        )
    ...
    def sync(self, query="*", from_date=None, in_memory=False):
        ...
        try:
            ...
            # collected so far, then let the caller see the error.
            self.end_sync(len(doc_updated), n_mef_updated, mef_errors, completed=False)
            raise
    ...
    def start_clean(self):
        ...
        self.start_timestamp = datetime.now()
        if self.dry_run:
            self.logger.info("--------- Starting cleaning in dry run mode ---------")
        else:
