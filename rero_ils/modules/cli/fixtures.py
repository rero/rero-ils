...
@click.option("-v", "--verbose/--no-verbose", "verbose", is_flag=True, default=True)
...
@click.option("-P", "--pid-check", "pid_check", is_flag=True, default=False)
...
def create(
    ...
    """
    click.secho(f"Loading {pid_type} records from {infile.name}.", fg="green")
        ...
        try:
            ...
            if create_or_update and db_record:
                ...
                if append:
                    pids.append(rec.pid)
            if verbose:
                click.echo(f"{count: <8} {pid_type} {msg} {rec.pid}:{rec.id}")

        except Exception as err:
            pid = record.get("pid", "???")
            click.secho(f"{count: <8} {type} create error {pid}: {err}", fg="red")
            if debug:
                traceback.print_exc()
...
def count_cli(infile, lazy):
    ...
    """
    click.secho(f"Count records from {infile.name}.", fg="green")
    records = read_json_record(infile) if lazy else json.load(infile)
...
@with_appcontext
def create_csv(record_type, json_file, output_directory, lazy, verbose, create_pid):
    """Create csv files from json.

    :param verbose: Verbose.
    """
    click.secho(f"Create CSV files for: {record_type} from: {json_file}", fg="green")

    path = current_jsonschemas.url_to_path(get_schema_for_resource(record_type))
...
def bulk_save(pid_types, output_directory, deployment, verbose):
    ...
    for p_type in pid_types:
        if p_type not in all_pid_types:
            click.secho(f"Error {p_type} does not exist!", fg="red")
            continue
