...
def checkout(item, data):
    ...
    :param data: additional data used for the circ operation (as a dict).
    """
    data["override_blocking"] = flask_request.args.get("override_blocking", False)
...
def item(item_barcode):
    ...
    item = Item.get_item_by_barcode(item_barcode, current_librarian.organisation_pid)
    if not item:
        abort(404)
    loan = get_loan_for_item(item_pid_to_object(item.pid))
    if loan:
        loan = Loan.get_record_by_pid(loan.get("pid")).dumps(LoanCirculationDumper())
    ...
    if patron_pid := flask_request.args.get("patron_pid"):
        ...
        if circ_policy.can_checkout:
            for action in item_dumps.get("actions", []):
                if action == "checkout":
                    if (
                        item.number_of_requests() > 0 and item.patron_request_rank(patron) == 1
...
def stats(item_pid):
    ...
    trigger = A(
        "terms",
        field="loan.trigger",
        aggs={"year": A("filter", Q("range", date={"gte": "now-1y"}))},
    )
