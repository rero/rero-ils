...
class ILLRequestSourceForm(FlaskForm):
    ...
    def validate(self, **kwargs):
        ...
        if self.url.data:
            self.origin.validators = [validators.DataRequired()]
        if self.origin.data:
            self.url.validators = [
...
class ILLRequestForm(FlaskForm):
    ...
    pages = StringField(label=_("Pages"))
    source = FormField(ILLRequestSourceForm, label=_("Found in"))
    note = TextAreaField(label="Note", render_kw={"rows": 5})
    ...
    def get_data(self):
        ...
        if self.note.data:
            data["notes"] = [{"type": "public_note", "content": self.note.data}]
