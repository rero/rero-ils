...
def doc_item_view_method(pid, record, template=None, **kwargs):
    ...
    query = HoldingsSearch().filter("term", document__pid=pid.pid_value)
...
def note_general(notes):
    ...
    return sort_by_type(list(filter(lambda t: t["noteType"] == "general", notes)))
...
def cartographic_attributes(attributes):
    ...
    return [
        ...
        if "projection" in attribute or attribute.get("coordinates", {}).get("label")
    ]
...
def provision_activity_publication(provisions):
    ...
    return {"bf:Publication": provisions.get("bf:Publication", [])}
...
def doc_entity_label(entity, language=None, part_separator=" - ") -> str:
    ...
    for subdivision in entity.get("subdivisions", []):
        if sub_entity := subdivision.get("entity"):
            _, _, label = doc_entity_label(sub_entity, language, part_separator)
            parts.append(label)
...
def part_of_format(part_of):
    ...
    # Set title
    bf_titles = list(filter(lambda t: t["type"] == "bf:Title", document.get("title")))
    ...
    if "restrict_pickup_to" in location:
        ...
        for library in org.get_libraries():
            for location_pid in list(library.get_pickup_locations_pids()):
                pickup_locations.append(Location.get_record_by_pid(location_pid))
    ...
    for work in work_access_point:
        ...
        if agent := work.get("creator"):
            if agent["type"] == EntityType.PERSON:
                ...
                if (
                    "numbering" in agent
                    or "conference_date" in agent
                    or "place" in agent
                ):
                    conf = [
                        agent[key] for key in ["numbering", "conference_date", "place"]
                    ]
                    if len(conf):
        ...
        if "medium_of_performance_for_music" in work:
            agent_formatted += f"{'. '.join(work['medium_of_performance_for_music'])}. "
...
def create_title_alternate_graphic(titles, responsibility_statement=None):
    ...
    """
    output = []
    altgr_texts = title_format_text_alternate_graphic(titles, responsibility_statement)
...
def create_title_variants(titles):
    ...
    """
    output = []
    title_variant_texts = title_variant_format_text(titles=titles, with_subtitle=True)
...
def online_holdings(document_pid, viewcode="global"):
    ...
    results = query.source(
        ["library", "electronic_location", "enumerationAndChronology", "notes"]
    ).scan()
...
def series_statement_format(series):
    ...
    return [SeriesStatementExtension.format_text(serie) for serie in series]
...
def main_title_text(title):
    ...
    """
    return list(filter(lambda t: t.get("type") == "bf:Title", title))
