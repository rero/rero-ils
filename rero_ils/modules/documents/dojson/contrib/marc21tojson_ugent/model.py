...
def marc21_to_type_and_issuance(self, key, value):
    ...
    type = []
    fields_920 = marc21.get_fields(tag='920')
    article = False
    if fields_920:
        subfields_920_a = marc21.get_subfields(fields_920[0], 'a')
        if subfields_920_a:
            if subfields_920_a[0] == 'article':
                type = [{
    ...
    if marc21.record_type == 'a' and article is False:
        if marc21.bib_level == 'm':
            type = [{
...
@utils.for_each_value
@utils.ignore_value
def marc21_to_intendedAudience(self, key, value):
...
def marc21_to_identifiedBy_from_field_022(self, key, value):
    ...
    identifiedBy = self.get('identifiedBy', [])
    for subfield_code in ['a', 'l', 'm', 'y', 'z']:
...
def marc21_to_identifiedBy_from_field_024(self, key, value):
    ...
    if subfield_a:
        if re.search(r'bnf\.fr/ark', subfield_a, re.IGNORECASE):
            identifier.update({
                'value': subfield_a,
                'type': 'uri',
                'source': 'BNF'
