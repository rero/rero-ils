...
def marc21_to_type_and_issuance(self, key, value):
    ...
    # if adminMetadata/descriptionLanguage contains
    # "Language of cataloging varies", remove it !
    try:
        if self['adminMetadata']['descriptionLanguage'] == \
                'Language of cataloging varies':
            self['adminMetadata']['descriptionLanguage'] = 'und'
    except Exception:
        self['adminMetadata']['descriptionLanguage'] = 'und'
    ...
    # Add a paragraph to insert a attribute "type" with
    # main_type and subtype
...
def marc21_to_language(self, key, value):
    ...
    valid_260 = False
    fields_260 = marc21.get_fields(tag='260')
    if fields_260:
        valid_260 = True
    ...
    if not valid_264 and not valid_260:
        if fields_264:
            error_print('WARNING INVALID 264', marc21.bib_id, marc21.rero_id,
                        fields_264)
        if fields_260:
            error_print('WARNING INVALID 260', marc21.bib_id, marc21.rero_id,
                        fields_260)
        places = []
        publication = {
            'type': 'bf:Publication'
...
@marc21.over('title', '^210..')
@utils.for_each_value
@utils.ignore_value
def marc21_to_abbreviated_title(self, key, value):
    """Get abbreviated title data."""
    title_list = self.get('title', [])
    if value.get('a'):
        title_part = [value.get('a')]
        if value.get('b'):
            title_part.append(value.get('b'))
        main_title = ' '.join(title_part)
...
@marc21.over('keytitle', '^222..')
@utils.for_each_value
@utils.ignore_value
def marc21_to_key_title(self, key, value):
    """Get key title data."""
    title_list = self.get('title', [])
    if value.get('a'):
        title_part = [value.get('a')]
...
def marc21_to_contribution(self, key, value):
    ...
    if not key[4] == '2' and key[:3] in ['100', '700', '710', '711']:
        ...
        if key[:3] == '700' and value.get('t'):
            agent = build_agent()
            work = {'title': value.get('t')}
            if agent:
                work['agent'] = agent
...
def marc21_to_usage_and_access_policy_from_field_506_540(self, key, value):
    ...
    if subfield_a:
        ...
        self['usageAndAccessPolicy'] = usage_and_access_policy
...
@utils.ignore_value
def marc21_to_identifiedBy_from_field_010(self, key, value):
...
def marc21_to_identifiedBy_from_field_020(self, key, value):
    ...
    def build_identifier_from(subfield_data, status=None):
        subfield_data = subfield_data.strip().replace('-', '')
        identifier = {'value': subfield_data}
...
def marc21_to_identifiedBy_from_field_022(self, key, value):
    ...
    identifiedBy = self.get('identifiedBy', [])
    identifier = {}
    for subfield_code in ['2', 'a', 'l', 'm', 'y', 'z']:
        ...
        if subfields_data:
            ...
            for subfield_data in subfields_data:
                ...
                if subfield_code != '2':
                    ...
                    identifier['qualifier'] = subfield_data
...
def marc21_to_identifiedBy_from_field_024(self, key, value):
    ...
    if subfield_a:
        if re.search(r'permalink\.snl\.ch', subfield_a, re.IGNORECASE):
            ...
            subfield_a = subfield_a.split('bnf.fr/')[1]
            identifier.update({
                'value': subfield_a,
                'type': 'uri',
                'source': 'BNF'
...
@utils.for_each_value
@utils.ignore_value
def marc21_to_notes_and_credits(self, key, value):
    """Get notes and original title."""
    subfield_a = None
    if value.get('a'):
        subfield_a = utils.force_list(value.get('a'))[0]
...
@utils.for_each_value
@utils.ignore_value
def marc21_to_supplementaryContent(self, key, value):
    """Get notes and original title."""
    if value.get('a'):
        subfield_a = utils.force_list(value.get('a'))[0]
