...
def delete_drafts(days=1, delete=False, verbose=False):
    ...
    count = len(pids)
    if verbose:
        click.secho(f"Delete drafts {days_ago} count: {len(pids)}", fg="yellow")
    for pid in pids:
        if doc := Document.get_record_by_pid(pid):
            ...
            if delete:
                try:
                    ...
                    count -= 1
                    msg = f"DELETE DRAFT: {pid} {doc.reasons_not_to_delete()}"
                    if verbose:
                        click.secho(f"ERROR: {msg}", fg="red")
                    current_app.logger.warning(msg)

    set_timestamp("clean_drafts", msg={"deleted": count})
    return count
