...
class UsersResource(ContentNegotiatedMethodView):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(
            method_serializers={
                "GET": {"application/json": json.dumps},
                "PUT": {"application/json": json.dumps},
            },
...
class UsersCreateResource(ContentNegotiatedMethodView):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(
            method_serializers={
                "GET": {"application/json": json.dumps},
                "POST": {"application/json": json.dumps},
            },
            serializers_query_aliases={"json": json.dumps},
            default_method_media_type={
                "GET": "application/json",
    ...
    def get(self):
        ...
        if email_or_username.startswith("email:"):
            user = User.get_by_email(email_or_username[len("email:") :])
        elif email_or_username.startswith("username:"):
            user = User.get_by_username(email_or_username[len("username:") :])
        else:
        ...
        # Don't return a dict, as we use a pure JSON serializer
        hits["hits"]["total"] = 1
        return hits
