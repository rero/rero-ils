...
def extracted_data_from_ref(input_, data="pid"):
    """Extract a data from a `$ref` string.

    :param input_: string where to search data, or a dict containing "$ref" key
    ...
    * extracted_data_from_ref({"$ref": "http://localhost/[resource]/[pid]"})
    ...
    def get_acronym():
        ...
        resource_list = extracted_data_from_ref(input_, data="resource")
        endpoints = {
    ...
    def get_record():
        ...
        pid = extracted_data_from_ref(input_, data="pid")
        record_class = extracted_data_from_ref(input_, data="record_class")
    ...
    switcher = {
        ...
        "pid": lambda: extract_part(input_.get("$ref"), -1),
        "resource": lambda: extract_part(input_.get("$ref"), -2),
        "acronym": get_acronym,
...
def number_records_in_file(json_file, type_):
    """Get number of records per file."""
    count = 0
    with open(json_file, buffering=1) as file:
        for line in file:
            if (type_ == "json" and '"pid"' in line) or type_ == "csv":
                count += 1
    return count
...
def get_all_roles():
    ...
    if access_ext := current_app.extensions["invenio-access"]:
        roles = [
            ...
            for role_name in access_ext.system_roles
