...
class MigrationDataListResource(ContentNegotiatedMethodView):
    ...
    def get(self, **kwargs):
        ...
        search.aggs.bucket("subset", "terms", field="deduplication.subset", size=30)
        search.aggs.bucket(
        ...
        search.aggs.bucket(
            "modified_by", "terms", field="deduplication.modified_by.keyword", size=30
        )
        ...
        if subset := flask_request.args.get("subset"):
            search = search.filter("term", deduplication__subset=subset)
        ...
        if modified_by := flask_request.args.get("modified_by"):
            search = search.filter(
                "term", deduplication__modified_by__keyword=modified_by
            )
        # sorting
...
class MigrationDataResource(ContentNegotiatedMethodView):
    ...
    def put(self, id):
        ...
        if not ils_pid:
            migration_data.deduplication.status = "no_match"
        else:
        ...
        migration_data.deduplication.modified_at = datetime.now()
        migration_data.save()
