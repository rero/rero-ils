...
class Import(object):
    ...
    def create_aggregations(self, results):
        ...
        if year_config := results.get("aggregations", {}).get("year", {}).get("config"):
            results["aggregations"] = {"year": {"config": year_config}}
        else:
            results["aggregations"] = {}
        year_min = int(datetime.now().strftime("%Y"))
        year_max = 1400
        for data in results["hits"]["hits"]:
            y_min, y_max = self.calculate_aggregations(data["metadata"], data["id"])
            ...
            for key, value in values.items():
                ...
                if subs := value.get("sub"):
                    ...
                    for sub_key, sub_value in subs.items():
                        ...
                        sub_buckets.sort(key=lambda e: (-e["doc_count"], e["key"]))
                        bucket_data[value.get("sub_type")] = {"buckets": sub_buckets}
        ...
        try:
            ...
            if cache and not no_cache:
                ...
                if self.results["hits"]["hits"]:
                    cache_data = {"hits": self.results["hits"], "data": self.data}
                    self.cache.setex(
                        cache_key,
...
class KULImport(Import):
    ...
    url_api = (
        "{url}/view/sru/32KUL_LIBIS_NETWORK?"
        "version=1.2&operation=searchRetrieve"
        "&recordSchema=marcxml&maximumRecords={max_results}"
        '&startRecord=1&query={where} {relation} "{what}"'
    )
...
class RenouvaudImport(Import):
    ...
    url_api = (
        "{url}/view/sru/41BCULAUSA_NETWORK?"
        "version=1.2&operation=searchRetrieve"
        "&recordSchema=marcxml&maximumRecords={max_results}"
        '&startRecord=1&query={where} {relation} "{what}"'
    )
