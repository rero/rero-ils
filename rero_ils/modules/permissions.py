...
def expose_action_needs_by_role(roles=None):
    ...
    def _perform_system_role(role_names):
        ...
        for row in query:
            matrix.setdefault(
                row.role_name,
                {"type": "system_role", "actions": deepcopy(actions_list)},
...
def expose_action_needs_by_patron(patron):
    ...
    role_ids = [r.id for r in roles_query.all()]
    query = ActionRoles.query.filter(ActionRoles.role_id.in_(role_ids))
    for row in query.all():
        with contextlib.suppress(KeyError):
            permissions_matrix[row.action]["reasons"][row.role.name] = not row.exclude

    # Load specific ActionUsers permission from Invenio-access and store them
    # into the permission_matrix.
    query = ActionUsers.query.filter(ActionUsers.user_id == patron.user.id)
    for row in query.all():
...
class AllowedByActionRestrictByOwnerOrOrganisation(AllowedByAction):
    ...
    def needs(self, record=None, *args, **kwargs):
        ...
        if record:
            ...
            if required_need and not required_need.intersection(g.identity.provides):
                return []
