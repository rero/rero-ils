    ...
    **dummy_kwargs,
    ...
    if index.split("-")[0] == HoldingsSearch.Meta.index:
        ...
        holding = next(
            ...
            .source("holdings_type")
            .scan(),
            None,
        )
        # get the number of items for ui paging
        item_search = ItemsSearch()[0:0].filter("term", holding__pid=record.pid)
...
def update_items_locations_and_types(sender, record=None, **kwargs):
    ...
    """
    if (
        isinstance(record, Holding)
        and record.get("holdings_type") == HoldingTypes.SERIAL
    ):
