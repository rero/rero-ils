...
class Holding(IlsRecord):
    ...
    def is_available(self):
        ...
        unavailable_item_pids = self.get_item_pids_with_active_loan(available_item_pids)
    ...
    def get_links_to_me(self, get_pids=False):
        ...
        links = {"items": items, "local_fields": local_fields}
    ...
    def reasons_not_to_delete(self):
        ...
        if self.is_serial:
            ...
            for item in self.get_all_items():
                ...
                reasons_not_to_delete.pop("local_fields", None)
                ...
                if cannot_delete_msgs := {
                    f"has {value} items with {name} attached": value
    ...
    def _get_next_issue_display_text(cls, patterns):
        ...
        for pattern in patterns.get("values", []):
            ...
            for level in pattern.get("levels", []):
                text_value = level.get("next_value", level.get("starting_value", 1))
    ...
    def _increment_next_prediction(cls, patterns):
        ...
        for pattern in patterns.get("values", []):
            for level in reversed(pattern.get("levels", [])):
                ...
                next_value = level.get("next_value", level.get("starting_value", 1))
                if max_value == next_value:
                    level["next_value"] = level.get("starting_value", 1)
                else:
    ...
    def prediction_issues_preview(self, predictions=1):
        ...
        if self.patterns and self.patterns.get("values"):
            ...
            for r in range(predictions):
                issue, expected_date = self._get_next_issue_display_text(patterns)
    ...
    def prediction_issues_preview_for_pattern(
        ...
        if patterns and patterns.get("values"):
            for r in range(number_of_predictions):
                issue, expected_date = cls._get_next_issue_display_text(patterns)
    ...
    def _prepare_issue_record(
        ...
        if status == ItemIssueStatus.RECEIVED:
            data["issue"]["received_date"] = datetime.now().strftime("%Y-%m-%d")
        if item:
    ...
    def create_regular_issue(self, status, item=None, dbcommit=False, reindex=False):
...
def get_holdings_by_document_item_type(document_pid, item_type_pid):
...
def create_holding(
    ...
    location_pid=None,
    item_type_pid=None,
    electronic_location=None,
    holdings_type=HoldingTypes.STANDARD,
    patterns=None,
    ...
    missing_issues=None,
    call_number=None,
    second_call_number=None,
    notes=None,
    vendor_pid=None,
    acquisition_status=None,
    acquisition_expected_end_date=None,
    acquisition_method=None,
    general_retention_policy=None,
    completeness=None,
    composite_copy_report=None,
    issue_binding=None,
    masked=False,
):
    ...
    data = {
        ...
        "circulation_category": {"$ref": get_ref_for_pid("itty", item_type_pid)},
    ...
    return Holding.create(data, dbcommit=False, reindex=False, delete_pid=False)
