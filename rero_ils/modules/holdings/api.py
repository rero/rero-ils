...
from ..api import IlsRecord, IlsRecordIndexer, IlsRecordError
...
class Holding(IlsRecord):
    ...
    @property
    def circulation_type_pid(self):
        """Shortcut for circulation type pid of the holding."""
        return self.replace_refs()['circulation_type']['pid']
...
def get_holding_pid_for_item(document_pid, location_pid, item_type_pid):
    ...
    result = HoldingsSearch().filter(
        ...
        'term',
        circulation_type__pid=item_type_pid
    ).filter(
        'term',
...
def create_holding_for_item(document_pid, location_pid,
                            item_type_pid):
