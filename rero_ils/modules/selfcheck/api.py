        ...
        with force_locale(language):
            ...
            if patron:
                ...
                patron_account_information = SelfcheckPatronInformation(
                    ...
                    patron_status=get_patron_status(patron),
                    institution_id=patron.organisation_pid,
                    language=patron.get("patron", {}).get(
                        "communication_language", "und"
                    ),
                    email=patron.get("patron", {}).get(
                        "additional_communication_email", patron_dumps.get("email")
                    ),
                ...
                for loan in get_loans_by_patron_pid(patron.pid, filter_states):
                    ...
                    if (
                        loan["state"] == LoanState.ITEM_ON_LOAN
                        and loan.is_loan_overdue()
                    ):
                        patron_account_information.setdefault(
                            "overdue_items", []
                ...
                if fee_amount > 0:
                    ...
                    for transaction_pid in transaction_pids:
                        ...
                        transaction = PatronTransaction.get_record_by_pid(
                            transaction_pid
                        )
    ...
    if check_sip2_module():
        ...
        with force_locale(language):
            if item:
                ...
                item_information["current_location"] = item.get_last_location().get(
                    "name"
                )
        ...
        with force_locale(language):
            try:
                ...
                item = Item.get_item_by_barcode(
                    barcode=item_barcode, organisation_pid=terminal.organisation_pid
                )
                if not item:
                ...
                if staffer.is_professional_user:
                    patron = Patron.get_patron_by_barcode(
                        barcode=patron_barcode, org_pid=terminal.organisation_pid
                    )
                    if not patron:
                ...
                checkout = SelfcheckCheckout(
                    title_id="", due_date=datetime.now(timezone.utc)
                )
