...
from rero_ils.modules.loans.api import (
    Loan,
    get_loans_by_item_pid_by_patron_pid,
...
from .utils import (
    authorize_selfckeck_terminal,
    authorize_selfckeck_user,
    check_sip2_module,
        ...
        with force_locale(language):
            ...
            if patron:
                ...
                patron_account_information = SelfcheckPatronInformation(
                    ...
                    patron_status=get_patron_status(patron),
                    institution_id=patron.organisation_pid,
                    language=patron.get("patron", {}).get("communication_language", "und"),
                    email=patron.get("patron", {}).get("additional_communication_email", patron_dumps.get("email")),
                ...
                for loan in get_loans_by_patron_pid(patron.pid, filter_states):
                    ...
                    if field := sip2_summary_fields.get(loan["state"]):
                        patron_account_information.setdefault(field, []).append(item.get("barcode"))
                    if loan["state"] == LoanState.ITEM_ON_LOAN and loan.is_loan_overdue():
                        patron_account_information.setdefault("overdue_items", []).append(item.get("barcode"))
                ...
                if fee_amount > 0:
                    ...
                    transaction_pids = get_transactions_pids_for_patron(patron.pid, status="open")
                    for transaction_pid in transaction_pids:
                        # TODO: return screen message to notify patron if there
                        #  are other open transactions
                        transaction = PatronTransaction.get_record_by_pid(transaction_pid)
    ...
    if check_sip2_module():
        ...
        with force_locale(language):
            if item := Item.get_item_by_barcode(item_barcode, org_pid):
                ...
                item_information["media_type"] = map_media_type(document["type"][0]["main_type"])
                ...
                item_information["current_location"] = item.get_last_location().get("name")
...
def selfcheck_checkout(transaction_user_pid, item_barcode, patron_barcode, **kwargs):
    ...
    if check_sip2_module():
        ...
        with force_locale(language):
            try:
                terminal = SelfcheckTerminal.find_terminal(name=kwargs.get("terminal"))
                item = Item.get_item_by_barcode(barcode=item_barcode, organisation_pid=terminal.organisation_pid)
                if not item:
                ...
                if staffer.is_professional_user:
                    patron = Patron.get_patron_by_barcode(barcode=patron_barcode, org_pid=terminal.organisation_pid)
                    if not patron:
                    ...
                    if LoanAction.CHECKOUT in data:
                        ...
                        if checkout_note:
                            checkout.get("screen_messages", []).append(checkout_note)
                ...
                checkout.get("screen_messages", []).append(_("No circulation action is possible"))
            except CirculationException as circ_err:
                checkout.get("screen_messages", []).append(_(circ_err.description))
            except Exception:
                checkout.get("screen_messages", []).append(_("Error encountered: please contact a librarian"))
    ...
    if check_sip2_module():
        ...
        with force_locale(language):
            try:
                terminal = SelfcheckTerminal.find_terminal(name=kwargs.get("terminal"))
                library = Library.get_record_by_pid(terminal.library_pid)
                checkin = SelfcheckCheckin(permanent_location=library.get("name"))
                item = Item.get_item_by_barcode(barcode=item_barcode, organisation_pid=terminal.organisation_pid)
                if not item:
                    raise ItemBarcodeNotFound

                document = Document.get_record_by_pid(item.document_pid)
                checkin["title_id"] = TitleExtension.format_text(document.get("title"))
                staffer = Patron.get_record_by_pid(transaction_user_pid)
                if staffer.is_professional_user:
                    # do checkin
                    ...
                    if LoanAction.CHECKIN in data:
                        ...
                        if checkin_note:
                            checkin.get("screen_messages", []).append(checkin_note)
                        # TODO: When is possible, try to return fields:
                        #       magnetic_media
                        # TODO: implements `print_line`
            except ItemBarcodeNotFound:
                checkin.get("screen_messages", []).append(_("Error encountered: item not found"))
                checkin.get("screen_messages", []).append(_("Error encountered: please contact a librarian"))
            except NoCirculationAction:
                checkin.get("screen_messages", []).append(_("No circulation action is possible"))
            except CirculationException as circ_err:
                checkin.get("screen_messages", []).append(_(circ_err.description))
            except Exception:
    ...
    if check_sip2_module():
        ...
        with force_locale(language):
            try:
                ...
                document = Document.get_record_by_pid(item.document_pid)
                renew = SelfcheckRenew(title_id=TitleExtension.format_text(document.get("title")))
                ...
                renew.get("screen_messages", []).append(_("Error encountered: item not found"))
                renew.get("screen_messages", []).append(_("Error encountered: please contact a librarian"))
            except NoCirculationAction:
                renew.get("screen_messages", []).append(_("No circulation action is possible"))
            except CirculationException as circ_err:
                renew.get("screen_messages", []).append(_(circ_err.description))
            except Exception:
                renew.get("screen_messages", []).append(_("Error encountered: please contact a librarian"))
