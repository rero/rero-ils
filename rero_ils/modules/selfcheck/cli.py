...
@with_appcontext
def create_terminal(name, user, location_pid, scopes, internal, access_token, comments):
    ...
    if SelfcheckTerminal.find_terminal(name=name):
        click.secho(f"ERROR terminal name already exist: {name}", fg="red")
        sys.exit(1)
...
@with_appcontext
def update_terminal(name, enable, disable, location_pid, access_token, comments):
