...
class Deduplication:
    ...
    def check_date(self, data, candidates):
        ...
        for candidate in candidates:
            ...
            if (prov and c_prov) and (prov == {k: c_prov[k] for k in prov if c_prov.get(k)}):
                new_candidates.append(candidate)
    ...
    def check_responsibility_statement(self, data, candidates):
        ...
        for candidate in candidates:
            ...
            if (not c_value and not value) or (
                (c_value and value) and self.normalize(value[0][0]["value"]) == self.normalize(c_value[0][0]["value"])
            ):
                new_candidates.append(candidate)
    ...
    def rescore(self, data, candidate):
        ...
        detailed_scores = {k: {"value": v[1], "weight": v[0]} for k, v in scores.items()}
        # combine the scores
        return (
