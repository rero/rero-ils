...
def upgrade():
    ...
    """
    query = (
        RecordsSearch(index=LoanOperationLog.index_name)
        .filter("term", record__type="illr")
        .filter("bool", must_not=[Q("exists", field="ill_request.library_pid")])
    )
    ...
    for idx, pid in enumerate(pids, 1):
        ...
        if ill_request := ILLRequest.get_record_by_pid(ill_request_pid):
            try:
                record["ill_request"]["library_pid"] = ill_request.get_library().pid
